...
import com.example.sid.pdfGenratorDemo.service.PdfAndExcelService;
...
@Service
public class PdfExcelAndCsvServiceImpl implements PdfAndExcelService {
	...
	private void createExcelDataRow(HSSFSheet workSheet, HSSFCellStyle bodyCellStyle, int i, Employee employee) {
		...
		HSSFCell cityValue = bodyRow.createCell(3);
	...
	public boolean createCsv(List<Employee> employees, ServletContext context, HttpServletRequest request,
		...
		try {
			...
			for (Employee employee : employees) {
				String[] dataRow = { Integer.toString(employee.getEmployeeId()), employee.getFirstName(),
						employee.getLastName(), employee.getCity() };
